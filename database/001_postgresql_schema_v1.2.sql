-- C5 Sentinel-SAR PostgreSQL / Supabase Schema v1.2
...
-- =========================================================
-- v1.2 FIELD EXPERIMENT + LOGISTICS HARDENING
-- =========================================================

do $$ begin
  create type logistics_movement_type as enum (
    'initial_load',
    'resupply_received',
    'treatment_consumed',
    'damaged_discarded',
...
create table if not exists inventory_ledger_v12 (
  id uuid primary key default gen_random_uuid(),
  incident_id uuid references incidents(id) on delete cascade,
  actor_id uuid references profiles(id),
  item_type text not null,
  movement_type logistics_movement_type not null,
  ...
  created_at timestamptz not null default now(),
  payload_json jsonb not null default '{}'::jsonb,
  constraint unique_device_logistics_event_v12 unique (device_id, local_event_id)
);

create index if not exists idx_ledger_v12_item_aggregation
on inventory_ledger_v12 (incident_id, item_type);

create or replace view vw_command_inventory_burn_rate_v12 as
select
  incident_id,
  item_type,
  sum(quantity_delta) as current_stock_available,
  abs(coalesce(sum(quantity_delta) filter (where quantity_delta < 0), 0)) as total_consumed_to_date,
  count(distinct actor_id) as active_medics_drawing_supplies
from inventory_ledger_v12
group by incident_id, item_type;

create or replace function trg_process_logistics_event_safety_v12()
returns trigger as $$
declare
...
begin
  ...
  from inventory_ledger_v12
  where incident_id = NEW.incident_id and item_type = NEW.item_type;
  ...
  if v_current_stock < 0 then
    ...
    values (
      ...
      'INVENTORY_NEGATIVE_STOCK_USED',
      'warning',
      'פריט באפס מלאי נמצא בשימוש בשטח. נדרש דחף אספקה מיידי.',
      jsonb_build_object(
        'item_type', NEW.item_type,
        'calculated_negative_stock', v_current_stock,
        'medic_id', NEW.actor_id,
...
drop trigger if exists trg_process_logistics_event_safety_v12 on inventory_ledger_v12;
create trigger trg_process_logistics_event_safety_v12
after insert on inventory_ledger_v12
for each row execute function trg_process_logistics_event_safety_v12();

create table if not exists experiment_observer_notes (
  id uuid primary key default gen_random_uuid(),
  incident_id uuid references incidents(id) on delete cascade,
  device_id text not null,
  actor_id uuid references profiles(id),
...
create table if not exists patient_edit_conflicts (
  id uuid primary key default gen_random_uuid(),
  incident_id uuid references incidents(id) on delete cascade,
  patient_id uuid references patients(id) on delete cascade,
  device_id text not null,
  opened_last_modified_at timestamptz,
  stored_last_modified_at timestamptz,
  conflict_type text not null default 'simultaneous_patient_edit',
  created_at timestamptz not null default now(),
  payload_json jsonb not null default '{}'::jsonb
);
