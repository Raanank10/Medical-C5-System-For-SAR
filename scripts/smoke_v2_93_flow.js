...
for (const file of files) {
  ...
  assert(html.includes('Demo V2.999'), `${rel}: launcher version label must be V2.999`);
  assert(html.includes('ROLE // V2.999'), `${rel}: role header version label must be V2.999`);
  assert(html.includes('Role-Based Medical Command System V2.999'), `${rel}: role system strip must be V2.999`);
