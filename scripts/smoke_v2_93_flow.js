...
for (const file of files) {
  ...
  assert(html.includes('Demo V2.998'), `${rel}: launcher version label must be V2.998`);
  assert(html.includes('ROLE // V2.998'), `${rel}: role header version label must be V2.998`);
  assert(html.includes('Role-Based Medical Command System V2.998'), `${rel}: role system strip must be V2.998`);
