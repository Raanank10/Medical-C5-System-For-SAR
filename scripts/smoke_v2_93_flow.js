...
for (const file of files) {
  ...
  assert(html.includes("roleSection('עומס חובשים'"), `${rel}: missing PC medic-load title`);
  ...
  assert(html.includes("roleSection('הצעות פיקוד — מחושב בזמן אמת', suggestionsHtml)"), `${rel}: CC live suggestions missing`);
  ...
  assert(html.includes("const APP_VERSION = '2.99.3';"), `${rel}: APP_VERSION must be V2.993`);
  assert(html.includes('Demo V2.993'), `${rel}: launcher version label must be V2.993`);
  assert(html.includes('ROLE // V2.993'), `${rel}: role header version label must be V2.993`);
  assert(html.includes('Role-Based Medical Command System V2.993'), `${rel}: role system strip must be V2.993`);
