...
for (const file of files) {
  ...
  assert(html.includes("const APP_VERSION = '2.96.0';"), `${rel}: APP_VERSION must be V2.96`);
  assert(html.includes('Demo V2.96'), `${rel}: launcher version label must be V2.96`);
  assert(html.includes('ROLE // V2.96'), `${rel}: role header version label must be V2.96`);
  assert(html.includes('Role-Based Medical Command System v2.96'), `${rel}: role system strip must be V2.96`);
