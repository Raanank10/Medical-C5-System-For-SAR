...
for (const file of files) {
  ...
  assert(renderBody.includes('דימום מסכן חיים?'), `${rel}: missing immediate hemorrhage section`);
  assert(renderBody.indexOf('דימום מסכן חיים?') < renderBody.indexOf('הולך?'), `${rel}: hemorrhage section must precede walking`);
  ...
  const mobilityLabel = '<div class="field-label">לכוד / ניידות</div>';
  ...
  console.log(`${rel}: V2.98 MSTART card static smoke passed`);
}
