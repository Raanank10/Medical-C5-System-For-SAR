...
for (const rel of files) {
  ...
  assert(html.includes("const APP_VERSION = '2.99.9';"), `${rel}: APP_VERSION must be 2.99.9`);
  assert(html.includes('Demo V2.999'), `${rel}: launcher label must be Demo V2.999`);
  assert(html.includes('ROLE // V2.999'), `${rel}: role header must be V2.999`);
  assert(html.includes('Role-Based Medical Command System V2.999'), `${rel}: role strip must be V2.999`);
  ...
  console.log(`${rel}: V2.999 demo polish static smoke passed`);
}
