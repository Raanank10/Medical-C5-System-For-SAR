...
for (const rel of files) {
  ...
  assert(html.includes("const APP_VERSION = '2.99.8';"), `${rel}: APP_VERSION must be 2.99.8`);
  assert(html.includes('Demo V2.998'), `${rel}: launcher label must be Demo V2.998`);
  assert(html.includes('ROLE // V2.998'), `${rel}: role header must be V2.998`);
  assert(html.includes('Role-Based Medical Command System V2.998'), `${rel}: role strip must be V2.998`);
  ...
  console.log(`${rel}: V2.998 demo polish static smoke passed`);
}
