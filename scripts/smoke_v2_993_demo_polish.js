...
for (const rel of files) {
  ...
  assert(html.includes("const APP_VERSION = '2.99.7';"), `${rel}: APP_VERSION must be 2.99.7`);
  assert(html.includes('Demo V2.997'), `${rel}: launcher label must be Demo V2.997`);
  assert(html.includes('ROLE // V2.997'), `${rel}: role header must be V2.997`);
  assert(html.includes('Role-Based Medical Command System V2.997'), `${rel}: role strip must be V2.997`);
  ...
  console.log(`${rel}: V2.997 demo polish static smoke passed`);
}
