...
for (const rel of files) {
  ...
  assert(html.includes("const APP_VERSION = '2.99.6';"), `${rel}: APP_VERSION must be 2.99.6`);
  assert(html.includes('Demo V2.996'), `${rel}: launcher label must be Demo V2.996`);
  assert(html.includes('ROLE // V2.996'), `${rel}: role header must be V2.996`);
  assert(html.includes('Role-Based Medical Command System V2.996'), `${rel}: role strip must be V2.996`);
  ...
  console.log(`${rel}: V2.996 demo polish static smoke passed`);
}
