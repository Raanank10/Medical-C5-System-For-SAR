...
for (const rel of files) {
  ...
  assert(html.includes("const APP_VERSION = '2.99.5';"), `${rel}: APP_VERSION must be 2.99.5`);
  assert(html.includes('Demo V2.995'), `${rel}: launcher label must be Demo V2.995`);
  assert(html.includes('ROLE // V2.995'), `${rel}: role header must be V2.995`);
  assert(html.includes('Role-Based Medical Command System V2.995'), `${rel}: role strip must be V2.995`);
  ...
  console.log(`${rel}: V2.995 demo polish static smoke passed`);
}
