-- C5 Sentinel-SAR PostgreSQL / Supabase Schema v0.7
...
do $$ begin
  create type event_type as enum (
    ...
    'TOURNIQUET_APPLIED','TOURNIQUET_RELEASED','TOURNIQUET_STATUS_RECORDED','INTERVENTION_RECORDED','MEDICATION_ADMINISTERED',
    ...
    'SYNC_CONFLICT','CONFLICT_LOG_ENTRY','TREATMENT_SAFETY_WARNING','HIGH_RISK_CLINICAL_VIOLATION','DEAD_MAN_SWITCH_ESCALATED','NOTE_ADDED'
  );
...
begin
  ...
  if new.type in ('MSTART_OVERRIDE','JUMPSTART_OVERRIDE','SYNC_CONFLICT','CONFLICT_LOG_ENTRY','TREATMENT_SAFETY_WARNING','HIGH_RISK_CLINICAL_VIOLATION') then
...
-- =========================================================
-- v0.7 VITALS STEPPER SUPPORT INDEXES
...
-- =========================================================
-- v0.7 PEDIATRIC MEDICATION SAFETY — DO NOT BLOCK CLINICAL SAVES
...
-- =========================================================
-- v0.7 TREATMENT UPDATE SUPPORT INDEXES
-- =========================================================
...
begin
  if new.type = 'TOURNIQUET_APPLIED' and new.patient_id is not null then
    insert into tourniquets (incident_id, patient_id, limb, applied_event_id, applied_at, is_active)
    values (
      new.incident_id,
      new.patient_id,
      coalesce(new.payload_json->>'limb','unknown'),
      new.id,
      coalesce((new.payload_json->>'application_time')::timestamptz, new.local_timestamp),
      true
    )
...
-- =========================================================
-- v0.7 DRAFT INCIDENT APPROVAL QUEUE
-- =========================================================
...
-- =========================================================
-- v0.7 SUPABASE ROW LEVEL SECURITY BASELINE
-- These policies are intentionally conservative. Service-role/backend jobs bypass RLS.
-- =========================================================
...
create policy incidents_read_authenticated on incidents for select to authenticated using (true);

drop policy if exists incidents_command_write on incidents;
create policy incidents_command_write on incidents for all to authenticated using (app.is_command_role()) with check (app.is_command_role());

drop policy if exists sectors_read_authenticated on sectors;
create policy sectors_read_authenticated on sectors for select to authenticated using (true);

drop policy if exists sectors_command_write on sectors;
create policy sectors_command_write on sectors for all to authenticated using (app.current_user_role() in ('medic','pc','cc','chamal','admin')) with check (app.current_user_role() in ('medic','pc','cc','chamal','admin'));
...
create policy patients_medic_pc_insert on patients for insert to authenticated with check (app.current_user_role() in ('medic','pc'));

drop policy if exists patients_medic_pc_update_projection on patients;
create policy patients_medic_pc_update_projection on patients for update to authenticated using (app.current_user_role() in ('medic','pc','cc','chamal','admin')) with check (app.current_user_role() in ('medic','pc','cc','chamal','admin'));
...
create policy supply_requests_read_authenticated on supply_requests for select to authenticated using (true);

drop policy if exists supply_requests_write_ops on supply_requests;
create policy supply_requests_write_ops on supply_requests for all to authenticated using (app.current_user_role() in ('medic','pc','logistics_officer','admin')) with check (app.current_user_role() in ('medic','pc','logistics_officer','admin'));

drop policy if exists inventory_read_authenticated on inventory_ledger;
create policy inventory_read_authenticated on inventory_ledger for select to authenticated using (true);

drop policy if exists inventory_write_ops on inventory_ledger;
create policy inventory_write_ops on inventory_ledger for insert to authenticated with check (app.current_user_role() in ('medic','pc','logistics_officer','admin'));

drop policy if exists watchdog_read_authenticated on watchdog_alerts;
create policy watchdog_read_authenticated on watchdog_alerts for select to authenticated using (true);

drop policy if exists watchdog_command_update on watchdog_alerts;
create policy watchdog_command_update on watchdog_alerts for update to authenticated using (app.current_user_role() in ('pc','cc','chamal','admin')) with check (app.current_user_role() in ('pc','cc','chamal','admin'));

drop policy if exists conflict_read_command on conflict_log;
create policy conflict_read_command on conflict_log for select to authenticated using (app.is_command_role());

drop policy if exists aar_read_command on aar_reports;
create policy aar_read_command on aar_reports for select to authenticated using (app.is_command_role());

drop policy if exists aar_write_command on aar_reports;
create policy aar_write_command on aar_reports for all to authenticated using (app.is_command_role()) with check (app.is_command_role());



-- =========================================================
-- v0.7 DEAD MAN'S SWITCH ESCALATION
