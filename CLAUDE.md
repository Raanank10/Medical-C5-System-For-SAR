...
- `const APP_VERSION = '2.99.22';` — bump this and the visible "Demo X.Y" / "ROLE // X.Y" label strings together when shipping a version; smoke scripts assert on these exact strings. No changelog doc has been written since `docs/CHANGELOG_v2.995.md` — add one when behavior changes, don't let the version string drift further ahead of the changelog series.
- `const ROLE_LABELS` / `const ROLE_DEFS` — defines the role-based views. Ten roles exist in the live `user_role` enum: `medic`, `pc` (חוג"ד — platoon commander), `cc` (מ״פ רפואה — company/command medical), `logistics`, `chamal` (command system), `rpc`/`rcc` (rescue-chain platoon/company commander — site-authority actions per `docs/ROLE_COMMAND_MODEL_v2.8.md`, added in `database/006`/`007`), `admin`, and `physician`/`paramedic` (added in `database/019`/`020` to resolve cross-device concurrent-edit conflict resolution by role-authority rank — `physician > paramedic > cc > pc > medic` — see `docs/CONFLICT_RESOLUTION_DECISION.md`; `physician` also holds sole authority to write `PATIENT_DEATH_CONFIRMED`, `database/024`-`026`).
...
`database/001_postgresql_schema_v1.2.sql` is the base schema (`create table if not exists`, no migration framework yet — Phase 4 plans to convert to real Supabase CLI migrations, see `docs/PHASE_4_PLAN.md` 4C). `002_seed_demo_data_v1.2.sql` is matching synthetic seed data, `003_mci_ui_alignment.sql` a small delta on top, and `004` through `037` are the incremental fixes applied directly against the live project during deployment/RLS-audit and feature work — each file's name says what it fixes; `004`-`017` cover the original RLS-audit pass (RLS gaps, `rpc`/`rcc` roles and their site-authority RLS, auto-create-profile-on-signup, new rescue-chain event types, the command-dashboard RPC, incident-scoping the sync-error read policy, locking out inactive profiles), `018`-`027` add the `physician`/`paramedic` roles, field-level conflict resolution, and physician-only death confirmation, `028`-`031` cover audit-retention redaction and command-state fixes, and `032`-`037` build the real supply-request/dispatch projection and kit-template management (client vocabulary reconciliation, RLS corrections). See `docs/ARCHITECTURE.md`'s "Backend Deployment Status" for the full narrative — it is kept current and is the source of truth over this summary if the two ever disagree. Older `_v1.1` files and `archive/v0.6`, `archive/v0.7` are kept for history — don't edit them; add a new numbered file instead.
