...
- [x] backup and incident response plan — `docs/BACKUP_AND_INCIDENT_RESPONSE_PLAN.md`. Planning document only, not a drilled/tested runbook — states what's live, what the current Free-tier Supabase plan's backup capability actually is (daily backups, ~24h RPO, no PITR), and a minimal detect/assess/recover/communicate outline; RTO, on-call process, and named contacts need a real operating organization this repo doesn't have.
