...
- This is a **plan**, not a locked spec. Four of six original open decisions (mobile framework, command-web framework, monorepo tooling, migration tooling) were made explicitly with the user before this document's last update — see the Decisions section below for what was chosen and why, and don't re-ask them. Two remain genuinely open (`services/projectors`' scope, `index.html` retirement timing) — raise those with the user before the sub-phase that needs them. This repo's established working pattern (see recent PR history) is: research, present findings, ask before ambiguous/architecturally significant choices, implement, verify live, ship in small focused PRs, wait for explicit merge approval each time. Continue that pattern for anything new that comes up during implementation, not just the two listed decisions — Phase 4 is exactly the kind of work where skipping the "ask" step is expensive to undo.
...
Two decisions remain genuinely open — raise these with the user before the sub-phase that needs them, don't assume either:

5. **`services/projectors`' actual scope** — see the inventory table above. If nothing needs projection logic beyond what a Postgres trigger can express, this target may not need to exist as a separate deployable service at all. Confirm with the user what (if anything) is actually meant here before inventing a service to satisfy a roadmap bullet literally. Blocks 4D.
