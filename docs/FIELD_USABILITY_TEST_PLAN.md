...
**Scenario script**: not a fixed script — this session should be scenario-driven (a synthetic MCI narrative with injects: new casualties discovered, a resource shortage, a patient deteriorating, a comms disruption) rather than a checklist, since the point is to surface emergent issues a step-by-step script wouldn't produce. Use `analytics/c5_sentinel_sar_analytics_v1_1/seed_demo_db.py`'s existing synthetic scenario as a starting template, not a from-scratch design.
