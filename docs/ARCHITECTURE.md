...
Not built: the Command Dashboard read APIs, Command Actions, and AAR API sections of `docs/API_SURFACE_v1.2.md` (section 5-7) - only the two mobile-critical sync endpoints (sections 2-3). `index.html` now has real login and a working push to `/sync/log` (see below), so it is no longer a purely local `localStorage` app, but pull/state-projection and most of the Command Dashboard surface remain unbuilt. Also not built: the `rpc`/`rcc` "tactical view scoped to my own platoon/company, no vitals/identity" requirement plus RCC's "limited" resupply visibility, which need a real platoon/company data model (a `platoons` table, `patients.assigned_platoon_id` or equivalent) that doesn't exist anywhere in this schema yet - `platoon` currently only appears as an inventory-ownership tag string (`owner_type = 'platoon_stock'`), never as an actual table or relationship.
