...
Kit templates closed that last bullet in four further passes. `database/035` reconciled `inventory_items` (`kit_template_items`' FK target) with the client's real item vocabulary via a new `client_item_key` column - live-testing found the live project's `inventory_items` table was empty entirely (`002`'s seed insert was never actually applied there; `treatment_catalog` showed the same 1-row-vs-10-seeded pattern), so the migration seeds the base catalog, not just reconciles it. `database/036` fixed a real RLS-vs-documented-authorization gap found while scoping this work: `cc` was documented (client `AUTH_MATRIX`, spec table) as able to manage kit templates but RLS never granted it; widened to add `cc`, while `physician` - also documented as `yes` in the client's `AUTH_MATRIX` but never intended per explicit product direction (physician stays treatment-focused) - was corrected client-side instead of widening RLS to match a wrong claim. `database/037` gave the client its first real read on `incidents.status` at all (extending `incident_command_state`, the same reused-not-duplicated read path as `get_incident_command_state`) - the client's local `siteData.incidentStatus` is a separate, never-synced concept with a different vocabulary entirely. Live-testing this caught a second real bug: the existing events-driven refresh trigger only fires for a fixed clinical-event allowlist that doesn't include anything that changes `incidents.status`, so status changes weren't reliably reflected at all; fixed with a second trigger directly on `incidents(status)`.

The kit-template management screen itself (`screen-kit-templates`) is a standalone CRUD screen, not wired into the resupply-request-creation flow (a deliberate scope decision - kit templates need to exist before anyone could pick one anyway). Unlike `supply_requests`, `kit_templates`/`kit_template_items` are plain reference/config tables, not part of the append-only events log, so the client reads and writes them directly via `supabase-js` (matching the existing `device_presence` upsert pattern) with RLS as the real enforcement - no projection trigger, no RPC needed since the tables aren't incident-scoped. Reachable from the Logistics Hub and from pc/cc's shared real-queue section, gated client-side by `canManageKitTemplates()` (mirroring `AUTH_MATRIX`) and `kitTemplatesEditLocked()` (fail-safe: an unknown incident status locks editing, same as an active one).
