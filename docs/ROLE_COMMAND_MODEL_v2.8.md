...
One row is a deliberate exception to the matrix's usual pattern: "Confirm patient death" shows `admin:'no'`, unlike every other row (admin is `'yes'` everywhere else, reflecting the Edge Function's blanket admin bypass in `isEventTypeAllowed()`). This isn't an oversight — live-verified: `events_insert_by_role`'s `PATIENT_DEATH_CONFIRMED` carve-out (`database/026`) is unconditional on `actor_role = 'physician'`, not OR'd with the general admin-role branch, so even an admin's push is accepted by the Edge Function's role check but then rejected by RLS itself. The matrix reflects real enforced behavior, not the app's usual "admin can do anything for demo/test purposes" narrative.
