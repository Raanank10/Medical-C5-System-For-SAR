...
`user_role` (Postgres enum, live): `medic`, `pc`, `logistics_officer`, `cc`, `chamal`, `admin`, `rpc`, `rcc` — 8 roles. `rpc`/`rcc` were added after the initial schema (`database/006_add_rpc_rcc_enum_values.sql`) for the platoon/company-commander site-authority actions in `docs/ROLE_COMMAND_MODEL_v2.8.md`. The client maps `logistics_officer` (server) to `logistics` (client-side role key) via `SERVER_ROLE_TO_CLIENT_ROLE` in `index.html` — the only naming mismatch between the two layers; every other role name matches exactly.
...
- `app.is_command_role()` — `current_user_role() in ('pc','cc','chamal','admin')`. Used by policies scoped to command-tier reads (e.g. `sync_ingestion_errors`, `aar_context_notes`).
