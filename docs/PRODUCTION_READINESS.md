...
- [~] Keep Chamal/command dashboards on `incident_command_state` — the sector/building/floor heatmap panel now genuinely reads the server snapshot as its primary source (`incident_command_state.state_json.sectors`, sourced from `vw_command_spatial_heatmap`, wired in by `refresh_incident_command_state()` in `database/030`/`031`) rather than the 100% locally-computed data it was mislabeled as "incident_command_state cache" before — verified live (a synthetic test patient inserted directly into `patients` showed up correctly in the refreshed `state_json.sectors`, then cleaned up with no residue since it had no associated `events` rows). Falls back to local computation only when offline/local-demo, which offline-first requires regardless of server-primary status. Still local-primary and out of scope for this pass: per-sector inventory (no server-side equivalent — the demo's local stock simulation and the real `inventory_ledger_v12` ledger use different item/owner models that don't line up, see `database/030`'s own scope note), reinforcement requests, device/sync counts, and per-patient watchdog detail, none of which the aggregate snapshot carries.
- [~] Add structured logging and audit traces — `conflict_log`, `sync_ingestion_errors`, and `watchdog_alerts` provide a real audit trail for their specific domains. Observability itself is now reviewed (`docs/OBSERVABILITY_REVIEW.md`): checked live that neither deployed Edge Function logged anything and the client had exactly one `console.error` call in ~8,600 lines. Implemented the zero-dependency piece of that review's recommendation — both `supabase/functions/sync-log/index.ts` and `supabase/functions/handover-consume/index.ts` now emit structured JSON log lines (counts/codes/ids only, never clinical text or secrets) at real decision points, captured by Supabase's already-running `edge_logs`, plus a top-level `try`/`catch` so an unexpected exception is logged instead of silently becoming a bare 500. Deployed live (`sync-log` v5, `handover-consume` v2), `get_advisors` re-checked clean. Still open: a client-side uncaught-error hook and any centralized/cross-service aggregation, both deliberately deferred — see the review doc for why.
...
- [~] Audit retention policy — `docs/AUDIT_AND_RETENTION_POLICY.md` proposes four retention classes and the deletion-must-be-audited principle. The two schema-driven mechanisms that don't require picking a retention duration are now real and scheduled (`database/028`/`029`): expired handover-token credentials and resolved sync-ingestion-error payloads are redacted daily via `pg_cron`, each redaction audited to a new `retention_actions` table before it happens. Verified live (real redaction, real idempotency, real audit row; a `PUBLIC`-execute gap in the first pass was caught via `get_advisors` and fixed). Still open: Class A/B's actual retention windows and Class C's window-based fallback all need real legal/records-governance input this doc doesn't invent.
