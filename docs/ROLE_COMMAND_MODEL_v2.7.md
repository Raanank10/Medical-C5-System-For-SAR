# Role-Based Medical Command Model V2.7

V2.7 keeps the prototype as a role-based medical command system and makes each casualty a visible field command object: triage color, source/confidence, missing data, timers, owner, and next action.

## V2.7 Ticket Contract

Every casualty/patient ticket should answer:

- Who is this casualty? Temporary ID or known name.
- What triage color is it? Dominant red/yellow/green/black/pending band.
- How was the color set? MSTART automatic, manual, override, or deterioration update.
- Where is the casualty? Site and current zone.
- Who owns it? Assigned medic/platoon.
- What phase is it in? Sweep, monitoring, evacuation, handed over.
- What life-saving actions exist? Tourniquet, airway, pressure dressing, other.
- What is missing? Assessment debt chips.
- What should the user do next? One primary next action.
...
2. `MSTART — סריקה מהירה`
   ...
   - Life-saving actions are the top of the card.
   - Walking auto-fills breathing, adequate pulse, AVPU A, not trapped, and green unless contradicted.
   - A/V/P implies the casualty is breathing, but V/P/U clears walking and raises priority.
   - Breathing-no prompts airway opening and breathing recheck.
...
## V2.7 Operational Surfaces

Medic operational view is intentionally limited to:

- `סריקה`
- `הפצועים שלי`
- `מוקד / אזור`

Advanced demo, AAR, logistics, export, observer, and command dashboards stay available from the launcher or command roles. They do not dominate the medic’s field screen.

The medic top line should always answer: site, zone, sweep status, medic name, and local sync/outbox state.

## Command Replacement For Notebook/Radio Fog

V2.7 makes the חוג"ד view the first place where paper visibly loses:

- active/completed MSTART sweeps by zone,
- missing full vitals after sweep,
- unknown identity,
- trapped status gaps,
- missing evacuation status,
- tourniquet limb missing,
- no reassessment after threshold,
- and action buttons for monitoring assignment, reinforcement, paramedic, evacuation, and zone patient list.

The מ״פ רפואה view is a company resource board, not a larger patient list:

- active platoons,
- active sites/zones,
- red/yellow burden,
- trapped/immobile burden,
- open חוג"ד requests,
- doctor/paramedic/death-certification queues,
- evacuation bottlenecks,
- equipment shortages affecting care,
- and cross-platoon allocation recommendations.
...
- Every supply-consuming action opens a `ציוד נצרך` bottom sheet for confirm/cancel/change quantity/request resupply.
...
מ״פ רפואה commands medical resources directly. The V2.7 demo should make this feel like a company resource command center, not a larger platoon dashboard.
...
V2.7 does not include a generic stretcher-team request type; evacuation support is handled through medical evacuation coordination.
