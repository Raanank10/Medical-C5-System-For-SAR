...
**Gate before starting any Phase 4 sub-phase**: (1) `docs/FIELD_USABILITY_TEST_PLAN.md`'s Session 2 (offline/intermittent-connectivity drill) has run at least once — even informally, two devices and a controllable network, without waiting for a fully staffed field exercise — and its result (real reconnect-to-sync latency, whether `docs/FAILURE_MODE_REVIEW.md`'s F1 fix holds under a live extended-offline stress test, whether F3's role-authority tie-break behaves as intended under a real live-device collision, not just SQL impersonation tests) is recorded — **still open**; and (2) `docs/FAILURE_MODE_REVIEW.md`'s F3 (cross-device concurrent edits to the same patient) has an explicit, documented resolution rule — **resolved and built**: `docs/CONFLICT_RESOLUTION_DECISION.md` records a hybrid field-level-merge + role-authority (physician > paramedic > cc > pc > medic) resolution, built ahead of Phase 4 per the user's explicit choice (`database/019`-`021`, live on the deployed Supabase project) rather than deferred to 4D/4E. The physician-role prerequisite the decision doc originally flagged is resolved — `physician` and `paramedic` are now real server roles. Phase 4 is exactly the point where undecided product behavior becomes expensive engineering ambiguity across two codebases instead of one; both gates exist to keep that from happening by neglect — one is now closed, the field-usability-drill gate is not.
...
- [x] Encrypted local storage design — `docs/THREAT_MODEL.md`'s T2 is fully mitigated: a device PIN (never persisted) derives an AES-256-GCM key via PBKDF2; `screen-pin-gate` gates every fresh load ahead of `screen-login`, identically for logged-in and local-demo modes. The Supabase session token itself is now included — the earlier suspected bootstrapping conflict (invite links on a brand-new device) turned out not to exist, confirmed by direct code trace before building it.
...
- [x] Failure-mode review for stale data and sync conflicts — `docs/FAILURE_MODE_REVIEW.md`. Found and fixed a real bug while writing it: the local sync outbox's 500-entry cap could silently drop still-unsynced clinical events on a long enough offline stretch (`persistOutbox()`, verified with 3 Playwright cases including a reproduction of the real data-loss scenario under the old logic). F3 (cross-device concurrent edits) is now built, not just decided — see `docs/CONFLICT_RESOLUTION_DECISION.md`. Remaining findings (F2, F4, F5) documented, not fixed here — they need Phase 4 architecture work or field-validation data this review doesn't have.
