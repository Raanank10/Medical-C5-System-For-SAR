# Medical Command Demo Script V2.7
...
Open `index.html` in a browser. Use a mobile-sized browser window first, then switch through the V2.7 launcher path.
...
2. `MSTART — סריקה מהירה`
   The medic opens a dedicated sweep screen, taps `פצוע נוסף באזור הזה`, sees life-saving actions first, marks one walking casualty and watches the app create a big green ticket with auto-fill source, then marks another casualty and applies a tourniquet.
...
10. Notebook proof
   Show `מה המחברת הייתה מפספסת?`: missing full vitals after sweep, active tourniquet timers, low supplies, automatic green walking classification, pending equipment requests, missing evacuation status, and death-certification queue.
