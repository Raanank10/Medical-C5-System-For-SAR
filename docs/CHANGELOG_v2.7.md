...
Demo V2.7 turns every casualty into a field command ticket: dominant triage color, source/confidence, assessment debt, next action, and a clearer “what the notebook missed” proof for commanders.

## V2.7 Field Command Tickets

- Added large triage bands to MSTART sweep tickets, monitoring cards, PC patient cards, commander rows, supply-linked references, and AAR timeline entries.
- Each ticket now carries the triage source/confidence: `MSTART אוטומטי`, `ידני`, `חריגה`, or `עדכון קדימות`, including visible override reason when available.
- Tickets now show zone, assigned medic, phase, trapped/mobility status, life-saving actions, tourniquet timer, last vitals/full-vitals gap, assessment debt chips, and one next required action.
- Field mode enlarges triage labels so red/yellow/green/black are readable from distance.

## V2.7 Automation And Contradictions

- Strengthened walking automation and contradiction cleanup across the rapid intake and MSTART sweep paths.
- Tourniquet and airway actions clear auto-green walking classification and surface a visible warning.
- Tourniquet limb capture no longer offers an unknown limb option; one of the four limbs must be selected.

## V2.7 Command/AAR Proof

- Added `מה המחברת הייתה מפספסת?` on the launcher and AAR insights.
- CC evacuation priority now explicitly bubbles tourniquets with less than 45 minutes remaining in the 2-hour window.
- AAR adds first green, auto-filled fields, manual override count, PC acknowledgement count, and CC resource assignment count.

## V2.7 Medic-First Field Flow
...
## V2.7 חוג"ד And מ״פ רפואה Boards
...
- Tourniquet capture now asks limb quickly and requires selecting one of the four limbs.
