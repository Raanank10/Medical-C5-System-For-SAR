# C5 Sentinel-SAR v0.7 Changelog

Changes applied from the reviewed draft spec:

1. Renamed the document from `FINAL SPECIFICATION v2.0` to `MVP Product & Technical Specification v0.7`.
...
2. Reworked sync model.
   - Removed `origin_device_id` as the conflict-resolution concept.
...
8. Removed UX/sync telemetry from clinical event enum.
   - `UX_METRIC_RECORDED` and `SYNC_ATTEMPT_RECORDED` are no longer clinical event types.
