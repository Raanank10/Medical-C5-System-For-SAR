# C5 Sentinel-SAR — MVP Product & Technical Specification v0.7
...
Out-of-stock blocks increment and prompts resupply.
...
# Treatment Update Protocol v0.7
...
If patient is pediatric and the treatment is medication-related, `weight_estimate_kg` is required.

If missing, the database rejects the event.
...
If the relevant inventory item is out of stock:

- the default behavior is hard block
- supervisor/PC override may be allowed in training/demo mode
- override requires reason and writes a conflict log entry
...
# v0.7 Corrections and Safety Updates
...
## Sync Echo Protection

Realtime updates include `server cursor / sync metadata`.

When the backend publishes an update from `realtime_outbox`, clients compare the message `server cursor / sync metadata` to their own device ID.

If they match, the client ignores the message because the device has already applied that local event. This prevents “sync echo,” where a device receives and re-applies its own server-confirmed update.
...
For pediatric patients, medication events require a `weight_estimate_kg`.

Database-level validation blocks pediatric medication events when `weight_estimate_kg` is missing.

The system still does not prescribe medication doses. It only records medication actions and forces the operator to provide the required weight-estimate context for pediatric cases.


---

# v0.7 Platoon Stock Logistics Layer
...
# v0.7 Draft Incident, Sync, Vitals, and KPI Updates
...
- Medics and PCs can create patients and clinical events.
...
Removed from clinical `event_type`:

```text
UX_METRIC_RECORDED
SYNC_ATTEMPT_RECORDED
```

If needed, UX and sync telemetry belong in separate analytics tables.
...
The active tourniquet projection is derived from these events. Offline release events stop watchdogs once synced.

## 9. AAR Generation

AAR generation is explicit:
...
Unlock does not magically generate the report; generation builds the report from immutable event history and KPI views.
